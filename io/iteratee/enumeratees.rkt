	...
	(λ: ((elem : (Stream D)))
	    (cond 
	     [(eq? elem 'Nothing)
	      (Continue (step inner))]
	     [(eq? elem 'EOS)
	      (Done 'EOS (icomplete inner))]
	     [else (match inner
			  [(Done _ _)
			   (Done elem (icomplete inner))]
			  [(Continue istep)
			   (begin
			     (for-fn elem)
...
(define (enumeratee/c-filter/map filter/map)  
  (λ: ((inner : (Iteratee I A)))    
	...
	(λ: ((elem : (Stream O)))
	    (cond
	     ((eq? elem 'Nothing) (Continue (step inner)))
	     ((eq? elem 'EOS)     (Done 'EOS (icomplete inner)))
	     (else                (match inner
					 [(Done _ _ ) (Done elem (icomplete inner))]
					 [(Continue istep)
					  (let ((new-elem (filter/map elem)))
					    (if new-elem
						(let ((newinner (istep new-elem)))
	    ...
	    (cond
	     ...
	     (else 
	      (let: loop : (Iteratee (Listof O) A)
			...
			(Continue (step inner))
			(match inner
			       [(Done _ _) (Done data (icomplete inner))]
			       [(Continue istep)
  ...
  (λ: ((inner : (Iteratee (Listof D) A)))
	...
	(λ: ((datum : (Stream D)))
	    (cond 
	     ...
	     ((eq? datum 'EOS)
	      (match inner
		     [(Done _ _ ) (Done 'EOS (icomplete inner))]
		     [(Continue inner-step)
		      (Done 'EOS (icomplete (inner-step accum)))]))
	     (else (match inner
			  [(Done _ _) (Done datum (icomplete inner))]
			  [(Continue inner-step)
			   (if (null? accum)
