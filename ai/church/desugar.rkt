...
(require
 (only-in "readable-scheme.rkt" tagged-list?)
  ...
  (define (loop-through-paths path-list)
    (if (null? path-list)
        ...
        (if (file-exists? (string-append (first path-list) filename))
            ...
            (loop-through-paths (cdr path-list)))))
...
(define (named-let->lambda expr)
  (let* ((proc-name (second expr))
         (let-conversion (let->lambda (cdr expr))))
    `((Y (lambda (,proc-name) ,(first let-conversion))) ,@(cdr let-conversion)) ))
    ...
    (if (null? bindings)
        ...
        (let* ((binding (car bindings))
          ...
          (if (null? (cdr bindings))
              ...
              `((lambda (,var) (let* ,(cdr bindings) ,body)) ,value-exprs) )))))
...
(define (desugar-begin-defines sexpr)
  (let* ((defines (map desugar-define-fn (filter (lambda (e) (tagged-list? e 'define)) (cdr sexpr))));;de-sugar here is to make defines be in standard form.
         (non-defines '(filter (lambda (e) (not (tagged-list? e 'define))) (cdr sexpr))))
    `(letrec ,(map cdr defines) ,(begin-wrap non-defines))))
...
(define (register-query-sugar query-name)
  (define (query? expr)
    (and (tagged-list? expr query-name)
         (>= (length (cdr expr)) 2))) ;;make sure not to try de-sugaring the definition of the query -- queries have at least two subexprs.
...
(define (make-lazy sexpr)
  (cond
    ...
    ((application? sexpr) `(,(make-lazy (first sexpr)) ,@(map delay-expr (cdr sexpr))))
    (else sexpr) ))
