...
(require 
 ...
 (only-in "header.rkt"     
          *intervention*
          ...
          church-make-initial-mcmc-state ;; Something not right here. RPR
          ...
          counterfactual-update
          addbox-empty?)
 (only-in "standard-preamble.rkt" 
          sum uniform-draw multinomial)
 (only-in "xrp-preamble.rkt" log-flip))
      ...
      (let ((proposal-xrps (proposable-xrps state proposable?)))
        (if (null? proposal-xrps)
            ...
            (let* ((chosen-xrp (uniform-draw proposal-xrps)) ;; xrp-draws is an addbox, so address is first.
                   ...
                   (ret2 (counterfactual-update state normal-form-proc (cons chosen-xrp proposed-val)))
...
(define (rejection-initializer normal-form-proc)
  ;; adding church- to make-initial-mcmc-state RPR.  Assumed the whole address store thing is injected as args here. 
  (let ((proposal-state (update->state (counterfactual-update (church-make-initial-mcmc-state) normal-form-proc)))) ;;don't care about bw/fw for init.
...
(define (annealing-initializer rej-steps temps:low->high temps->nfqp . rejuv-kernel-builder)
  (let* ([rejuv-kernel-builder (if (null? rejuv-kernel-builder) (lambda (nfqp) (basic-repeat-kernel rej-steps nfqp)) (first rejuv-kernel-builder))]
         ...
         [initial-state (begin (church-reset-store-xrp-draws)
                               (rejection-initializer normal-form-proc))])
...
(define (make-mh-kernel proposal-distribution scorer)
  (lambda (state)
    (let* ((ret (proposal-distribution state))
           ...
           (dummy (church-reset-store-xrp-draws))) ;;FIXME!! this is to avoid accumulating xrp-draws...
      (if accept
          proposal-state
          state))))
...
(define (with-times proc)
  (begin
    (for-each display (list "start: " (current-milliseconds) "\n"))
    (let ([return-value (proc)])
      (for-each display (list "done: " (current-milliseconds) "\n"))
      return-value)))
...
(define (mcmc-loop kernel state state->sample samples-left samples)
  (if (< samples-left 1)
      ...
      (mcmc-loop kernel
                 ...
                 state->sample
                 (- samples-left 1)
                 (cons (state->sample state) samples))))

(define (mh-query samples lag normal-form-proc)
  (repeated-mcmc-query-core (lambda () 
                              (rejection-initializer normal-form-proc))
  ...
  (lambda (state)
    (let* ((grad (mcmc-state->gradient state))
               ...
               (xrp-changes (map (lambda (gv) (let ((xrp-draw (first (pull-outof-addbox xrp-draws (first gv)))))
                                                (cons xrp-draw (+ (xrp-draw-value xrp-draw) (* step-size (sigmoid (rest gv)))))))
                                         grad))
               (proposal (apply counterfactual-update (cons state (cons nfqp xrp-changes)))))
    ...
    (let loop ((state (rejection-initializer nfqp))
      ...
      (if (< steps-remaining 1)
          ...
          (let ((newstate (gstep state)))
            (loop newstate
                  (cons (mcmc-state->query-value newstate) trail)
                  (- steps-remaining 1)))))))
  ...
  (let* ([xrp-draws (mcmc-state->xrp-draws state)]
         [xrp-changes (alist-map (lambda (addr inc)
                                   (let ([xrp-draw (first (pull-outof-addbox xrp-draws addr))])
                                     (cons xrp-draw (+ (xrp-draw-value xrp-draw) inc))))
                                 increments)])
    (apply counterfactual-update
           (cons state (cons nfqp xrp-changes)))))
...
(define (leapfrog-step nfqp q a:p m-inv dt)
  (let* ([addresses (map first a:p)]
         ...
         [qn/bwfw (increment-state q nfqp (map cons addresses increments))]
...
(define (smc-core temps popsize lag temps->nfqp)
  (let smc ((temps temps) ;;FIXME: rest temps?
            (population (repeat popsize (lambda () (begin
                                                     (church-reset-store-xrp-draws) ;;FIXME!! this is to avoid accumulating xrp-draws...
...
(define (mcmc-state->counterfactual-value state normal-form-proc)
  (let ([counterfactual-state (update->state (church-with-interventions state (lambda () (counterfactual-update state normal-form-proc))))])
