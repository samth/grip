...
(provide make-xrp 
         ...
         *intervention*
         ...
         church-make-initial-mcmc-state  ;; compiler must inject args in *.church code.
         church-reset-store-xrp-draws
         mcmc-state->score)

(require
 (only-in "AD.rkt" xy-gradient-R Tape? tapify)
 (only-in srfi/1 delete-duplicates fold)
 (only-in "utils.rkt" repeat)
 (only-in "trie.rkt" 
          make-empty-trie trie-insert trie-pop 
...
;         (def-symbols (map (lambda (d) (if (pair? (second d)) (car (second d)) (second d)))
...
;;  (fold (lambda (x y) (or x y)) #f args)) ;;FIXME: better way to do this? ;;FIXME!! doesn't return store..
...
(define (make-xrp address store xrp-name sample incr-stats decr-stats score init-stats hyperparams proposer support)
  ;,(if *lazy*
