...
(provide
 sum
 uniform-draw multinomial)

;; mostly dummied up stuff RPR
(require 
 (only-in "utils.rkt" mem)
 (only-in "sample.rkt"
          dirichlet
          sample-integer
          sample-discrete)
 (only-in "header.rkt" lev-dist)
 (only-in "xrp-preamble.rkt" flip))
...
(define (all lst) (andmap lst)) ;used to be (fold and true lst))
...
(define (any lst) (ormap lst));(fold or false lst))
...
(define (fold proc init . lsts)
  (if (null? (first lsts))
      init
      (apply fold (cons proc (cons (apply proc (append (single-map first lsts) (list init)))
...
(define (multi-map proc lsts) ;;takes list of lists and proc of that many arguments.
  (if (null? (car lsts)) 
      '() 
      (cons (apply proc (single-map first lsts)) 
...
(define (filter-map proc lst)
  (if (null? lst)
      ...
      (let ([val (proc (first lst))])
        (if val
            (cons val (filter-map proc (rest lst)))
