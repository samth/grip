...
(require 
 ...
 (only-in "header.rkt"
          make-xrp)
 (only-in "AD.rkt" untapify tapify)
 (only-in "sample.rkt" 
          random-real sample-real 
          random-integer lnfact
          sample-gaussian gaussian-lnpdf
          sample-dirichlet dirichlet-lnpdf 
          discrete-sampler discrete-pdf))

;; RPR found both of these in scheme-compiler/header-xxx.sc
(define *with-score-gradient* #f)
(define (continuous? x) (and (real? x)
                             (not (fixnum? x))))
...
(define gensym (make-stateless-xrp 'gensym
                                   (lambda prefix (apply gensym prefix))
...
(define (make-CRP alpha) (make-xrp 'CRP
                                   (lambda (stats hyperparam args) ;;sampler
                                     (let* ((count-map stats)
                                            (counts (cons hyperparam (map rest count-map)))
                                            (total-counts (apply + counts))
                                            (probs (map (lambda (c) (/ c total-counts)) counts))
                                       ...
                                       (if (= table-index 0)
                                           ...
                                           (let* ((table-symbol (gensym));(make-table-label))
                                                  (new-count-map ;(sort-stats
                                                   (cons (cons table-symbol 1) count-map)))
                                             (list table-symbol
                                                   new-count-map
                                           ...
                                           (let* ((table-symbol (first (list-ref count-map (- table-index 1))))
                                                  ...
                                                  (new-count-map ;(sort-stats
                                                   (append (take count-map (- table-index 1))
                                                           (list (cons table-symbol table-count))
                                           ...
                                           (let* ((table-count (rest (list-ref count-map table-index)))
                                                  ...
                                                  (new-count-map ;(sort-stats
                                                   (append (take count-map table-index)
                                                           (list (cons value new-table-count))
