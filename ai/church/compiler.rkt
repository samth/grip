...
(require
 racket/pretty
 (only-in "readable-scheme.rkt" tagged-list? repeat pair)
  ...
  (cond
    ...
    ((lambda? sexpr) (free-variables (lambda-body sexpr) (let loop ((params (lambda-parameters sexpr)))
                                                           (if (null? params)
                                                               bound-vars
                                                               (if (pair? params)
                                                                   (pair (car params) (loop (cdr params)))
                                                                   (pair params bound-vars))))))
    ((if? sexpr)  (apply append (map (lambda (e) (free-variables e bound-vars)) (cdr sexpr))))
