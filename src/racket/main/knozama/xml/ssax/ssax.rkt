...
(require
 ...
 (only-in knozama/xml/ssax/parser-error
	  parser-error ssax:warn)
 
 (only-in knozama/xml/ssax/look-for-str
	  find-string-from-port?)
 
 (only-in knozama/xml/ssax/char-encoding
	  ascii->char ucscode->char char-return)
 
 (only-in knozama/xml/ssax/input-parse
	  skip-until skip-while next-token next-token-of
	  read-string peek-next-char assert-curr-char)
 
 (only-in knozama/xml/sxml/env
	  nl))

(define (make-xml-token kind head)
  (cons kind head))
...
(define-syntax xml-token-kind
  (syntax-rules ()
    ((xml-token-kind token) (car token))))

(define-syntax xml-token-head
  (syntax-rules ()
    ((xml-token-head token) (cdr token))))
...
(define ssax:Prefix-XML
  (string->symbol "xml"))

(define name-compare
  (letrec ((symbol-compare (lambda (symb1 symb2)
			   (cond ((eq? symb1 symb2)
				  (quote =))
				 ((string<? (symbol->string symb1)
					    (symbol->string symb2))
				  (quote <))
				 (else (quote >))))))
    (lambda (name1 name2)
...
(define ssax:read-markup-token
  (let ()
	...
	((#\/) (read-char port)
	 (begin0
	     (make-xml-token (quote END)
			     (ssax:read-QName port))
	   (ssax:skip-S port)
	   (assert-curr-char (quote (#\>)) "XML [42]" port)))
	((#\?) (read-char port)
	 (make-xml-token (quote PI)
			 (ssax:read-NCName port)))
	((#\!) (case (peek-next-char port)
		 ((#\-) (read-char port) (skip-comment port))
		 ((#\[) (read-char port)
		  (read-cdata port))
		 (else (make-xml-token (quote DECL)
				       (ssax:read-NCName port)))))
	(else (make-xml-token (quote START)
			      (ssax:read-QName port)))))))
...
(define (ssax:read-pi-body-as-string port)
  (ssax:skip-S port)
  (string-concatenate/shared (let loop ()
			       (let ((pi-fragment (next-token (quote ())
							    (quote (#\?)) "reading PI content" port)))
				 (if (eqv? #\> (peek-next-char port))
				    (begin (read-char port) (cons pi-fragment (quote ())))
	  ...
	  (case (read-char port) ((#\newline)
		...
		((#\])
		 (if (not (eqv? (peek-char port) #\]))
		    ...
		    (let check-after-second-braket ((seed (if (string-null? fragment)
							   seed (str-handler fragment "" seed))))
		      (case (peek-next-char port) ((#\>)
						   (read-char port) seed)
		...
		(else (when (eqv? (peek-char port) #\newline)
			(read-char port))
		      (loop (str-handler fragment nl seed)))))))))

(define (ssax:read-char-ref port)
  (let* ((base (cond ((eqv? (peek-char port) #\x)
		    (read-char port) 16) (else 10)))
  ...
  (cond ((assq name entities) =>
	 (lambda (decl-entity)
	   (let ((ent-body (cdr decl-entity))
	     ...
	     (cond
	      ((string? ent-body)
	       (call-with-input-string ent-body (lambda (port)
	...
	((assq name ssax:predefined-parsed-entities) => (lambda (decl-entity)
...
(define ssax:read-attributes
  (let ((value-delimeters (append ssax:S-chars (quote (#\< #\&)))))
	...
	(cond ((or (eof-object? cterm)
		  (eqv? cterm delimiter)) new-fragments)
		 ...
		 (read-char port))
	      ...
	      ((eqv? cterm #\&)
	       (cond ((eqv? (peek-char port) #\#)
		     ...
		     (else (read-attrib-value delimiter port entities
	...
	(ssax:handle-parsed-entity port name entities
				   ...
				   (lambda (str1 str2 fragments)
				     (if (equal? "" str2)
					(cons str1 fragments)
					(cons* str2 str1 fragments))) fragments)))
	...
	(if (not (ssax:ncname-starting-char? (ssax:skip-S port)))
	   ...
	   (let ((name (ssax:read-QName port)))
	     ...
	     (let ((delimiter (assert-curr-char (quote (#\' #\")) "XML [10]" port)))
	       (loop (or (attlist-add attr-list (cons name (string-concatenate-reverse/shared
  ...
  (cond ((pair? unres-name)
	 (cons (cond
		...
		(else (parser-error port "[nsc-NSDeclared] broken; prefix " (car unres-name))))
	       (cdr unres-name)))
	(apply-default-ns?
	 (let ((default-ns (assq (quote *DEFAULT*) namespaces)))
	   (if (and default-ns (cadr default-ns))
	      (cons (cadr default-ns) unres-name) unres-name)))
	(else unres-name)))
  ...
  (let ((xmlns (string->symbol "xmlns"))
	...
	(if (attlist-null? attlist)
	   ...
	   (let*-values (((attr attr-others)
	     ...
	     (case (name-compare (car attr) (car decl-attr))
	       ...
	       (else (let*-values (((attr-name content-type use-type default-value)
				  (apply values decl-attr)))
	...
	(cond ((null? attrs) (values proper-attrs namespaces))
	      ((eq? xmlns (caar attrs))
	       (loop (cdr attrs) proper-attrs (if (equal? "" (cdar attrs))
						 ...
						 (add-ns port (quote *DEFAULT*)
							 (cdar attrs) namespaces))))
	      ((and (pair? (caar attrs))
		  (eq? xmlns (caaar attrs)))
	       (loop (cdr attrs) proper-attrs (add-ns port (cdaar attrs)
						      (cdar attrs) namespaces)))
	...
	(values (ssax:resolve-name port tag-head namespaces #t)
		(fold-right (lambda (name-value attlist)
			      (or (attlist-add attlist (cons (ssax:resolve-name port (car name-value) namespaces #f)
							    (cdr name-value)))
				 (parser-error port "[uniqattspec] after NS expansion broken for " name-value)))
			    (make-empty-attlist) proper-attrs) namespaces elem-content)))))
...
(define (ssax:read-external-id port)
  (let ((discriminator (ssax:read-NCName port)))
	...
	(let* ((delimiter (assert-curr-char (quote (#\' #\")) "XML [11]" port))
	     (systemid (next-token (quote ())
				   (list delimiter) "XML [11]" port)))
	  (read-char port) systemid))
    ...
    (cond ((eof-object? c) c)
	  ...
	  (else (let ((token (ssax:read-markup-token port)))
		  (case (xml-token-kind token)
		    ...
		    ((PI DECL START) token)
		    (else (parser-error port "XML [22], unexpected token of kind "
  ...
  (let ((terminators-usual (list #\< #\& char-return))
    ...
    (lambda (port expect-eof? str-handler seed)
	 ...
	 (let ((char-data-terminators
	      (if expect-eof?
		 terminators-usual-eof terminators-usual)))
	   (let loop ((seed seed))
		  ...
		  (case term-char
		    ((#\<) (let ((token (ssax:read-markup-token port)))
				...
				(loop (ssax:read-cdata-body port str-handler (handle-fragment fragment str-handler seed))))
			       ((COMMENT) (loop (handle-fragment fragment str-handler seed)))
			       (else (values (handle-fragment fragment str-handler seed) token)))))
		    ((#\&) (case (peek-next-char port)
			     ((#\#) (read-char port)
		    ...
		    (else (when (eqv? (peek-next-char port) #\newline)
			    (read-char port))
...
(define-syntax ssax:make-pi-parser
  (syntax-rules ()
    ((ssax:make-pi-parser orig-handlers)
     (letrec-syntax ((loop (syntax-rules (*DEFAULT*)
			   ((loop () #f accum port target seed)
			    (make-case
			     ((else (ssax:warn port "Skipping PI: " target nl)
				    (ssax:skip-pi port) seed) . accum) () target))
			   ((loop () default accum port target seed) (make-case
								      ((else (default port target seed)) . accum) () target))
  ...
  (syntax-rules ()
    ((ssax:make-elem-parser my-new-level-seed my-finish-element my-char-data-handler my-pi-handlers)
     (lambda (start-tag-head port elems entities namespaces preserve-ws? seed)
       (define xml-space-gi
	 (cons ssax:Prefix-XML (string->symbol "space")))
       (let handle-start-tag ((start-tag-head start-tag-head) (port port) (entities entities)
			    (namespaces namespaces)
	 ...
	 (let*-values (((elem-gi attributes namespaces expected-content)
	   ...
	   (case expected-content
	     ...
	     ((EMPTY) (ssax:assert-token
		       ...
		       (quote END) start-tag-head (lambda (token exp-kind exp-head)
						    (parser-error port "[elementvalid] broken for " token " while expecting " exp-kind exp-head)))
	      (my-finish-element elem-gi attributes namespaces parent-seed seed))
	     (else (let ((preserve-ws? (cond ((assoc xml-space-gi attributes) =>
					   ...
					   (else preserve-ws?))))
		     (let loop ((port port)
			      (entities entities) (expect-eof? #f) (seed seed))
		       (let*-values (((seed term-token)
				    (ssax:read-char-data port expect-eof? my-char-data-handler seed)))
			 (display (format "Seed: ~s Token ~s~n" seed term-token))
			 (if (eof-object? term-token)
			    seed
			    (case (xml-token-kind term-token)
			      ((END) (ssax:assert-token term-token
							(quote END) start-tag-head
							(lambda (token exp-kind exp-head)
							  (parser-error port "[GIMatch] broken for " term-token " while expecting " exp-kind exp-head)))
			       (my-finish-element elem-gi attributes namespaces parent-seed seed))
			      ((PI) (let ((seed ((ssax:make-pi-parser my-pi-handlers) port (xml-token-head term-token) seed)))
				      (loop port entities expect-eof? seed)))
			      ((ENTITY-REF) (let ((seed (ssax:handle-parsed-entity port (xml-token-head term-token) entities
										 (lambda (port entities seed) (loop port entities #t seed))
										 my-char-data-handler seed)))
					      (loop port entities expect-eof? seed)))
			      ((START) (when (eq? expected-content (quote PCDATA))
					 (parser-error port "[elementvalid] broken for " elem-gi " with char content only; unexpected token " term-token))
			       (let ((seed (handle-start-tag (xml-token-head term-token) port entities namespaces preserve-ws? seed)))
				 (loop port entities expect-eof? seed)))
...
(define-syntax ssax:make-parser/positional-args
  ...
  (syntax-rules ()
    ((ssax:make-parser/positional-args *handler-DOCTYPE *handler-UNDECL-ROOT *handler-DECL-ROOT *handler-NEW-LEVEL-SEED *handler-FINISH-ELEMENT *handler-CHAR-DATA-HANDLER *handler-PI)
     (lambda (port seed)
	 ...
	 (let*-values (((docname) (ssax:read-QName port))
		     ((systemid) (and (ssax:ncname-starting-char? (ssax:skip-S port))
	 ...
	 (let ((token (ssax:scan-Misc port)))
	   (if (eof-object? token)
	      ...
	      (case (xml-token-kind token)
		((PI)
		 (let ((seed ((ssax:make-pi-parser *handler-PI) port
			    (xml-token-head token) seed)))
		   (scan-for-significant-prolog-token-1 port seed)))
		((DECL)
		 (handle-decl port (xml-token-head token) seed))
		((START)
	 ...
	 (let ((token (ssax:scan-Misc port)))
	   (if (eof-object? token)
		...
		((PI) (let ((seed ((ssax:make-pi-parser *handler-PI) port (xml-token-head token) seed)))
			(scan-for-significant-prolog-token-2 port elems entities namespaces seed)))
		((START) (element-parser (xml-token-head token) port elems entities namespaces #f
					 (*handler-DECL-ROOT (xml-token-head token) seed)))
		(else (parser-error port "XML [22], unexpected markup " token))))))
       
       (define element-parser
	 (ssax:make-elem-parser *handler-NEW-LEVEL-SEED *handler-FINISH-ELEMENT *handler-CHAR-DATA-HANDLER *handler-PI))
...
(define-syntax ssax:define-labeled-arg-macro
  (syntax-rules ()
    ((ssax:define-labeled-arg-macro labeled-arg-macro-name (positional-macro-name (arg-name . arg-def) ...))     
     (define-syntax labeled-arg-macro-name (syntax-rules ()
					((labeled-arg-macro-name . kw-val-pairs)
					 (letrec-syntax ((find (syntax-rules (arg-name ...)
							       ((find k-args (arg-name . default) arg-name val . others)
								(next val . k-args)) ... ((find k-args key arg-no-match-name val . others)
...
(ssax:define-labeled-arg-macro ssax:make-parser
				...
				(DOCTYPE (lambda (port docname systemid internal-subset? seed)
					   (when internal-subset? (ssax:warn port "Internal DTD subset is not currently handled ")
	...
	(else (let loop ((fragments fragments) (result (quote ())) (strs (quote ())))
		(cond ((null? fragments)
		      ...
		      (else (loop (cdr fragments) (cons (car fragments)
							(if (null? strs)
							   result (cons (string-concatenate/shared strs) result)))
				  (quote ()))))))))

(define (ssax:reverse-collect-str-drop-ws fragments)
  (cond ((null? fragments) (quote ()))
	((null? (cdr fragments))
	 (if (and (string? (car fragments))
	       (string-whitespace? (car fragments)))
	    (quote ()) fragments))
	(else (let loop ((fragments fragments)
		       (result (quote ()))
		       (strs (quote ()))
		       (all-whitespace? #t))
		(cond ((null? fragments)
		       (if all-whitespace? result (cons (string-concatenate/shared strs) result)))
		      ((string? (car fragments))
		       (loop (cdr fragments) result (cons (car fragments) strs)
...
(define (xml->sxml port namespace-prefix-assig)
  (letrec ((namespaces (map (lambda (el) (cons* #f (car el)
					 (ssax:uri-string->symbol (cdr el))))
			  namespace-prefix-assig))
	 (RES-NAME->SXML (lambda (res-name)
			   (string->symbol (string-append (symbol->string (car res-name))
							  ":" (symbol->string (cdr res-name)))))))
    (let ((result (reverse ((ssax:make-parser NEW-LEVEL-SEED
					    (lambda (elem-gi attributes namespaces expected-content seed)
					      (quote ())) FINISH-ELEMENT (lambda (elem-gi attributes namespaces parent-seed seed)
									   (let ((seed (ssax:reverse-collect-str-drop-ws seed))
									       (attrs (attlist-fold (lambda (attr accum)
												      (cons (list (if (symbol? (car attr))
														     (car attr) (RES-NAME->SXML (car attr)))
														  (cdr attr)) accum)) (quote ()) attributes)))
									     (cons (cons (if (symbol? elem-gi)
											    elem-gi
											    (RES-NAME->SXML elem-gi))
											 (if (null? attrs)
											    seed
											    (cons (cons (quote *) attrs) seed))) parent-seed)))
					      CHAR-DATA-HANDLER (lambda (string1 string2 seed) (if (string-null? string2)
											     (cons string1 seed)
					      ...
					      DOCTYPE (lambda (port docname systemid internal-subset? seed)
							(when internal-subset?
							  ...
							  (ssax:skip-internal-dtd port))
							(ssax:warn port "DOCTYPE DECL " docname " " systemid " found and skipped")
							(values #f (quote ()) namespaces seed)) UNDECL-ROOT
							(lambda (elem-gi seed)
							  (values #f (quote ()) namespaces seed)) PI
							  ((*DEFAULT* lambda (port pi-tag seed) (cons (list (quote *PI*) pi-tag (ssax:read-pi-body-as-string port)) seed)))) port
							  (quote ())))))
      (cons (quote *TOP*)
